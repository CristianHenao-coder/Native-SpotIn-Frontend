...
import { getDistanceFromLatLonInM, TARGET_LOCATION } from '../../constants/location';
...
export default function Dashboard() {
  ...
  const handleCheckLocation = async () => {
    try {
      ...
      if (distance <= TARGET_LOCATION.radiusMeters) {
        ...
        Alert.alert('Success', `You are at the institution (${Math.round(distance)}m).`);
      } else {
        setIsValidLocation(false);
        Alert.alert('Too Far', `You are ${Math.round(distance)}m away. Move closer!`);
      }
    } catch (error) {
  ...
  const handleMarkAttendance = async () => {
      ...
      Alert.alert('Attendance Marked!', `User: ${user?.name}\nLocation: Verified\nQR: Verified`);
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 20, backgroundColor: '#f3f4f6' }}>
      ...
      <View style={{ gap: 16 }}>
        ...
        <View style={styles.card}>
            ...
            <TouchableOpacity
                style={[styles.button, isQrScanned ? styles.buttonSuccess : styles.buttonPrimary]}
                onPress={() => {
                    if (!permission.granted) requestPermission();
                    setIsScannerOpen(true);
                }}
            >
             <Text style={styles.buttonText}>{isQrScanned ? '✅ QR Scanned' : '📸 Scan QR'}</Text>
            </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
            ...
            <TouchableOpacity
                style={[styles.button, isValidLocation ? styles.buttonSuccess : styles.buttonPrimary]}
                onPress={handleCheckLocation}
            >
             <Text style={styles.buttonText}>{isValidLocation ? '✅ Location Verified' : '📍 Check Location'}</Text>
            </TouchableOpacity>
            {currentDistance !== null && (
                <Text style={{textAlign: 'center', marginTop: 8, color: '#6b7280'}}>
                    Distance: {currentDistance}m (Max: {TARGET_LOCATION.radiusMeters}m)
                </Text>
            )}
        </View>
        ...
        <View style={styles.card}>
             ...
             <TouchableOpacity
                ...
                style={[
                    styles.button, 
                    (isQrScanned && isValidLocation) ? styles.buttonAction : styles.buttonDisabled
                ]}
            ...
            >
             ...
             {(!isQrScanned || !isValidLocation) && (
                <Text style={{textAlign: 'center', marginTop: 8, color: '#ef4444', fontSize: 12}}>
                    Complete Step 1 & 2 first
                </Text>
            )}
        </View>

        <TouchableOpacity onPress={handleLogout} style={[styles.button, { backgroundColor: '#ef4444', marginTop: 20 }]}>
        ...
        <View style={{ flex: 1 }}>
            ...
            >
                <View style={{flex: 1, justifyContent: 'flex-end', padding: 50}}>
                    <Button title="Close Scanner" onPress={() => setIsScannerOpen(false)} color="red" />
                </View>
            </CameraView>
        </View>
      </Modal>
